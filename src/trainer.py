...
class Trainer():
    ...
    def train_one_epoch(self):
        ...
        for step, batch in enumerate(self.train_dataloader):
            logits, labels = self.model.train(*batch)
            loss = self.loss(logits, labels)
            total_loss += loss.detach().cpu().item()
            
            delta_tp, delta_tn, delta_fp, delta_fn = self.evaluate(logits.detach().cpu(), labels.detach().cpu())
            tp += delta_tp
            tn += delta_tn
            fp += delta_fp
            fn += delta_fn
            _, _, _, batch_f1 = self.metric(tp, tn, fp, fn)
            ...
            if (step != 0 and step % self.config.gradient_accumulation_steps == 0) or step == len(self.train_dataloader) - 1:
                ...
                progress_bar.set_postfix(
                    {
                        'loss': loss.detach().cpu().item() / (self.config.gradient_accumulation_steps * self.config.batch_size),
                        'f1': batch_f1,
                    }
                )
                progress_bar.update(1)
        self.metric(tp, tn, fp, fn)
    ...
    def validate(self):
        ...
        for step, batch in tqdm(enumerate(self.val_dataloader)):
            ...
            fp += delta_fp
            fn += delta_fn
        accu, prec, reca, f1 = self.metric(tp, tn, fp, fn)
        self.logger.info(f"Accuracy: {accu}")
        self.logger.info(f"Preision: {prec}")
        self.logger.info(f"Recall: {reca}")
        self.logger.info(f"F1: {f1}")
        return total_loss / step, f1
    
    @torch.no_grad()
    def evaluate(self, logits, labels):
        tp, tn, fp, fn = 0, 0, 0, 0
        # logits = torch.argmax(logits, dim=1)
        for pred, truth in zip(logits, labels):
            if truth == 0:
                if pred == 0:
                    tn += 1
                else:
...
class RankingMultipleNegativeTrainer():
    ...
    def train_one_epoch(self):
        ...
        for step, batch in enumerate(self.train_dataloader):
            ...
            # labels = [1] * len(answers_0) + [0.75] * len(answers_1) + [0.5] * len(answers_2) + [0] * len(answers_3)
            # labels = torch.tensor(labels)
            # labels = torch.stack([labels] * len(contexts)).to(self.config.device)
